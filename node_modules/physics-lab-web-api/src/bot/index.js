...
class Bot extends User {
  ...
  async get(once) {
    const re = await this.messages.get(this.targetID, this.targetType, 100);
    const that = this;
    const promises = []; // 用于存储每个评论处理的Promise
    ...
    for (let comment of re.Data.Comments) {
        ...
        if (
            ...
            !comment.Content.includes(this.botID)
        )
            continue;
        if (
            !comment.Content.includes(this.botID) &&
        ...
        // 处理评论，并添加到Promise数组中
        const promise = this.processFunction(comment, this)
            .then(reply => {
                if (reply !== "") {
                    const msg = `回复@${comment.Nickname}: ${reply}`;
                    return this.messages.comment(this.targetID, msg, this.targetType, comment.UserID);
                }
            })
            .then(() => {
                console.log("回复成功");
    ...
    if(once){
      process.exitCode = 0; // 设置退出状态为成功
  		process.exit(); 
    }
}
  async start(interval,once = false) {

    interval = Math.max(4000, Math.min(interval * 1000, 4000));

    const data = await this.messages.get(this.targetID, this.targetType, 20);
    console.log(this.replyConfig.readHistory == true);
    if (this.replyConfig.readHistory == true) {

      let index = data.Data.Comments.length >= 1 ? data.Data.Comments[data.Data.Comments.length-1].ID : "";;
    ...
    once && this.get(once)
    once || setInterval(this.get.bind(this), interval);
  }
}
module.exports = Bot;
